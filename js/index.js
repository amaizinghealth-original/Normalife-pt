...
function codeCheck(){
    ...
    $button.on("click", function (ev) {
        ...
        if (codeValues.test(codeValue) && $codeLenght==15){
            return $message.text("Este código é autêntico. Obrigado por ter escolhido o nosso produto!");
        } else if ($code.val() !== '' && $code.val() !== ' ' && $code.val() !== '  '){
            return $message.text("Infelizmente o código não foi achado! Muito provavelmente que Você comprou um produto falsificado.");
        }else{
            return $message.text("Introduza o código, por favor");
        }
